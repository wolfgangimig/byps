...
namespace byps { namespace http {
      ...
      if (var.isException()) {
        ...
        if (respBytes && respBytes->length) {
          ...
          try {
            ...
            std::vector<PContentStream> streams;
  ...
  BINLINE void HWireClient::internalSend(const PMessage& msg, PAsyncResult asyncResult, int32_t timeoutSecondsRequest) {
    ...
    if (nbOfRequests > 1) {
      ...
      for (unsigned i = 0; i < msg->streams.size(); i++) {
        PContentStream stream = msg->streams[i];

        if (!stream) continue;

        BTargetId targetId = stream->getTargetId();
        std::wstringstream ssurl;
        ssurl << url << L"?serverid=" << targetId.getServerId() << "&messageid=" << targetId.getMessageId() << L"&streamid=" << targetId.getStreamId();
        ...
        streamRequest->send(stream, streamResult);
      }
    }
    else {
  ...
  BINLINE void HWireClient::internalSendStreamsThenMessage(const PMessage& msg, PAsyncResult asyncResult, int32_t timeoutSecondsRequest) {
    ...
    if (nbOfStreams) {
      ...
      for (unsigned i = 0; i < msg->streams.size(); i++) {
        ...
        BTargetId targetId = stream->getTargetId();
        std::wstringstream ssurl;
        ssurl << url << L"?serverid=" << targetId.getServerId() << L"&messageid=" << targetId.getMessageId() << L"&streamid=" << targetId.getStreamId();
  ...
  public:
    ...
    virtual PContentStream openStream() {
      ...
      if (httpClient) {
        ...
        std::wstringstream ssurl;
        ssurl << url 
          << L"?serverid=" << targetId.getServerId() 
          << L"&messageid=" << targetId.getMessageId()
          << L"&streamid=" << targetId.getStreamId();
  ...
  BINLINE PContentStream HWireClient::getStream(const BTargetId& targetId) {
    l_debug << L"getStream(" << targetId;
    MyContentStream* stream = new MyContentStream(url, requestsToCancel, targetId, httpClient, timeoutSecondsClient);
