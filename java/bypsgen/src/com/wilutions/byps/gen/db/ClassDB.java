...
public class ClassDB {
     ...
     * @param fullName, qualified name plus type arguments, e.g. java.util.ArrayList&lt;Integer&gt;
	...
	public SerialInfo createSerialInfo(String name, List<CommentInfo> comments, String qname, String baseQname, 
			String dims, List<TypeInfo> typeArgs, List<MemberInfo> members, 
			boolean isEnum, boolean isFinal, boolean inlineInstance) throws GeneratorException {
		log.debug("createSerialInfo: " + qname + ", baseQname=" + baseQname + ", dims=" + dims + ", typeArgs=" + typeArgs + ", members=" + members + ", inlineInstance=" + inlineInstance + ", isEnum=" + isEnum);
		SerialInfo serInfo = new SerialInfo(name, comments, qname, baseQname, 
				dims, typeArgs, members, 
				isEnum, isFinal, inlineInstance);
		return serInfo;
	}

	public RemoteInfo createRemoteInfo(String name, List<CommentInfo> comments, String qname, List<MethodInfo> methods) throws GeneratorException {
		
		RemoteInfo rinfo = new RemoteInfo(name, comments, qname, methods);
	...
	public SerialInfo createStubForRemote(RemoteInfo rinfo)
		...
		SerialInfo sinfo = createSerialInfo(stubName, null, 
				stubQname, baseQname, "", null, 
				stubMembers, 
				false, false, false);
		add(sinfo);
	...
	public TypeInfo createTypeInfo(String name, String qname, String dims, List<TypeInfo> typeArgs,
			boolean isEnum, boolean isFinal, boolean isInline) throws GeneratorException {
...
//
//		if (typeId == 0) {
//			throw new GeneratorException("Failed to generate unique type ID. Please specify serialVersionUID in all your classes or switch to a higher binary model");
//		}
//		
...
//						registry.getMinTypeIdUser());
	...
	public void validate() throws BException {
		...
		if (!ret) {
			throw new BException(BException.GENERATOR_EXCEPTION, "Validate classDB failed. See error(s) printed before.");
		}
	}
