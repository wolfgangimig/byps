...
public abstract class HRemoteRegistry implements BServerRegistry {
	...
	public BClient getForwardClientIfForeignTargetId(BTargetId targetId) throws RemoteException {
		...
		BClient client = null;
		if (targetId.serverId != config.getMyServerId()) {    			
			client = getForwardClient(targetId.serverId);
			if (client == null) {
				throw new BException(BExceptionC.INTERNAL, "Cannot reach server for targetId=" + targetId +", missing URL or wrong URL for serverId=" + targetId.serverId + ", in configuration."); 
			}
		}
	...
	public BRemote getRemote(BTargetId targetId, int remoteId) throws RemoteException {
		...
		int serverId = targetId.serverId; 
