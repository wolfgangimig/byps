package com.wilutions.byps.test.api.list;

/*
 * 
 * THIS FILE HAS BEEN GENERATED. DO NOT MODIFY.
 */

import com.wilutions.byps.*;

/**
*/
@SuppressWarnings("all")
public class ListTypes implements BSerializable {

	public java.util.List<java.lang.Boolean> boolean1;
	public java.util.List<java.lang.Byte> byte1;
	public java.util.List<java.lang.Character> char1;
	public java.util.List<java.lang.Short> short1;
	public java.util.List<java.lang.Integer> int1;
	public java.util.List<java.lang.Long> long1;
	public java.util.List<java.lang.Float> float1;
	public java.util.List<java.lang.Double> double1;
	public java.util.List<java.lang.String> string1;
	public java.util.List<com.wilutions.byps.test.api.prim.PrimitiveTypes> primitiveTypes1;
	public java.util.List<byte[]> byte2;
	public java.util.List<int[]> int2;
	public java.util.List<int[][][][]> int4;
	public java.util.List<java.lang.Object> obj1;
	
	private final static long serialVersionUID = 3001L;
	
	public ListTypes() {
	}	
	
	public ListTypes(java.util.List<java.lang.Boolean> boolean1, java.util.List<java.lang.Byte> byte1, java.util.List<java.lang.Character> char1, java.util.List<java.lang.Short> short1, java.util.List<java.lang.Integer> int1, java.util.List<java.lang.Long> long1, java.util.List<java.lang.Float> float1, java.util.List<java.lang.Double> double1, java.util.List<java.lang.String> string1, java.util.List<com.wilutions.byps.test.api.prim.PrimitiveTypes> primitiveTypes1, java.util.List<byte[]> byte2, java.util.List<int[]> int2, java.util.List<int[][][][]> int4, java.util.List<java.lang.Object> obj1) {
		this.boolean1 = boolean1;
		this.byte1 = byte1;
		this.char1 = char1;
		this.short1 = short1;
		this.int1 = int1;
		this.long1 = long1;
		this.float1 = float1;
		this.double1 = double1;
		this.string1 = string1;
		this.primitiveTypes1 = primitiveTypes1;
		this.byte2 = byte2;
		this.int2 = int2;
		this.int4 = int4;
		this.obj1 = obj1;
	}	
	
}
