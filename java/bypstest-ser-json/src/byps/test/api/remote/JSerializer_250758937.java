package byps.test.api.remote;

/*
 * Serializer for byps.test.api.remote.BRequest_RemoteSetTypes_setPrimitiveTypes1
 * 
 * THIS FILE HAS BEEN GENERATED BY class byps.gen.j.GenSerStructJson DO NOT MODIFY.
 */

import byps.*;

// DEBUG
// isEnum=false
// isFinal=true
// isInline=false
// #members=1

// checkpoint byps.gen.j.GenSerStruct:304
@SuppressWarnings("all")
public class JSerializer_250758937 extends JSerializer_Object {
	
	public final static BSerializer instance = new JSerializer_250758937();
	
	public JSerializer_250758937() {
		super(250758937);
	}
	
	public JSerializer_250758937(int typeId) {
		super(typeId);
	}
	
	
	@Override
	public void internalWrite(final Object obj1, final BOutputJson bout, final BBufferJson bbuf) throws BException {
		final BRequest_RemoteSetTypes_setPrimitiveTypes1 obj = (BRequest_RemoteSetTypes_setPrimitiveTypes1)obj1;		
		bout.writeObj("primitiveTypes1",obj.primitiveTypes1, false, byps.test.api.JSerializer_673917574.instance);
	}
	
	@Override
	public Object internalRead(final Object obj1, final BInputJson bin) throws BException {
		final BRequest_RemoteSetTypes_setPrimitiveTypes1 obj = (BRequest_RemoteSetTypes_setPrimitiveTypes1)(obj1 != null ? obj1 : bin.onObjectCreated(new BRequest_RemoteSetTypes_setPrimitiveTypes1()));
		
		final BJsonObject js = bin.currentObject;
		obj.primitiveTypes1 = (java.util.Set<byps.test.api.prim.PrimitiveTypes>)bin.readObj("primitiveTypes1", false, byps.test.api.JSerializer_673917574.instance);
		
		return obj;
	}
	
	@Override
	public void prepareForLazyLoading(final Object obj1, final BInput bin, final long version) throws BException {
		final BRequest_RemoteSetTypes_setPrimitiveTypes1 obj = (BRequest_RemoteSetTypes_setPrimitiveTypes1)(obj1);		
		bin.prepareForLazyLoading(obj.primitiveTypes1, byps.test.api.BSerializer_673917574.instance);
		
	}
	
}
