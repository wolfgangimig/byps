...
public class MyRemoteConstants extends BSkeleton_RemoteConstants {
	...
	private <T> void compare(T lhs, T rhs) throws BException {
		if (!lhs.equals(rhs)) throw new BException(BException.INTERNAL, "Wrong constant");
	}
	
	private void compareArray(Object[] lhs, Object[] rhs) throws BException {
		if (!Arrays.equals(lhs, rhs)) throw new BException(BException.INTERNAL, "Wrong constant");
	}
	...
	private void compareArray4(int[][][][] lhs, int[][][][] rhs) throws BException {
		if (lhs.length != rhs.length) throw new BException(BException.INTERNAL, "Wrong constant");
		for (int i3 = 0; i3 < lhs.length; i3++) {
			int[][][] lhs2 = lhs[i3];
			int[][][] rhs2 = rhs[i3];
			if (lhs2.length != rhs2.length) throw new BException(BException.INTERNAL, "Wrong constant");
			for (int i2 = 0; i2 < lhs.length; i2++) {
				int[][] lhs1 = lhs2[i2];
				int[][] rhs1 = rhs2[i2];
				if (lhs1.length != rhs1.length) throw new BException(BException.INTERNAL, "Wrong constant");
				for (int i1 = 0; i1 < lhs.length; i1++) {
					int[] lhs0 = lhs1[i1];
					int[] rhs0 = rhs1[i1];
					if (!Arrays.equals(lhs0, rhs0)) throw new BException(BException.INTERNAL, "Wrong constant");
				}
			}
		}
	}
