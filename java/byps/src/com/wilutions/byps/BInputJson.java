...
public class BInputJson extends BInput {
	...
	public Object load() throws BException {
		...
		if (jsonMessageObj == null) throw new BException(BExceptionC.CORRUPT, "Missing message data.");
		if (!(jsonMessageObj instanceof BJsonObject)) throw new BException(BExceptionC.CORRUPT, "Message data must be a JSON object.");
		...
		if (headerObj == null) throw new BException(BExceptionC.CORRUPT, "Missing message header.");
		if (!(headerObj instanceof BJsonObject)) throw new BException(BExceptionC.CORRUPT, "Message header must be a JSON object.");
		...
		if (objectTableObj != null) {
			if (!(objectTableObj instanceof BJsonObject)) throw new BException(BExceptionC.CORRUPT, "Object table must be a JSON object.");
			objectTable = (BJsonObject)objectTableObj;
			
			Object objNull = objectTable.get(0);
			if (objNull != null) throw new BException(BExceptionC.CORRUPT, "First object table element must be null.");
	...
	protected Integer deserializeReference(String name, int idx) throws BException {
		...
		if (!(jsonObj instanceof BJsonObject)) {
			...
			throw new BException(BExceptionC.CORRUPT, msg);
		}
	...
	protected Object deserializeCurrentObject(BSerializer ser, Integer id)
		...
		if (ser == null) {
			...
			if (typeId <= 0) throw new BException(BExceptionC.CORRUPT, "Invalid _typeId=" + typeId + " for objectTable[" + id + "]");
